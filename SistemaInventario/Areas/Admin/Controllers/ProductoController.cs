    ...
    public class ProductoController : Controller
    {
        ...
        public async Task<IActionResult> Upsert(int? id)
        {
            ProductoVM productoVM = new ProductoVM()
            {
                ...
                CategoriaLista = _unidadTrabajo.Producto
                                               .ObtenerTodosDropdownLista("Categoria"),
                MarcaLista = _unidadTrabajo.Producto
                                           .ObtenerTodosDropdownLista("Marca"),
                PadreLista = _unidadTrabajo.Producto
                                           .ObtenerTodosDropdownLista("Producto")
            };
            if ( id == null )
            {
        ...
        {
            var todos = await _unidadTrabajo.Producto
                                            .ObtenerTodos( 
        ...
        {
            var productoDb = await _unidadTrabajo.Producto
                                               .Obtener(id);
            if (productoDb == null)
            {
                return Json( new { 
                                    success = false, 
                                    message = "Error al borrar Bodega"
                                 } 
                            );
            }
            //Remover Imagen
        ...
        {
            bool valor = false;
            var lista = await _unidadTrabajo.Producto
                                            .ObtenerTodos();
            if (id == 0)
            {
                valor = lista.Any(b => b.NumeroSerie
                                        .ToLower()
                                        .Trim() == serie.ToLower()
                                                         .Trim()
                                  );
