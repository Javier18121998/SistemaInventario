...
namespace SistemaInventario.Areas.Admin.Controllers
{
    ...
    public class MarcaController : Controller
    {
        ...
        public async Task<IActionResult> Upsert(int? id)
        {
            ...
            //Actualizamos Bodega
            marca = await _unidadTrabajo.Marca
        ...
        public async Task<IActionResult> Upsert(Marca marca)
        {
            if (ModelState.IsValid)
            {
                if (marca.Id == 0)
                {
                    await _unidadTrabajo.Marca
                                        .Agregar(marca);
                    TempData[DS.Exitosa] = "Marca creada Exitosamente";
                }
                else
                {
                    _unidadTrabajo.Marca
                                  .Acutaliza(marca);
        ...
        {
            var todos = await _unidadTrabajo.Marca
                                            .ObtenerTodos();
            return Json(new { data = todos });
        }
        public async Task<IActionResult> Delete( int id )
        {
            var marcaDb = await _unidadTrabajo.Marca
                                                  .Obtener(id);
            if (marcaDb == null)
            {
                return Json( new { 
            ...
            return Json(new { 
                                success = true,
                                message = "Marca borrada Exitosamente"
                            }
                        );
        }
        ...
        {
            bool valor = false;
            var lista = await _unidadTrabajo.Marca
                                            .ObtenerTodos();
            if (id == 0)
            {
